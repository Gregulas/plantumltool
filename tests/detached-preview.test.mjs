...
test('creates a serializable detached preview update', () => {
  const message = detachedPreviewState({ svg: '<svg></svg>', filename: 'flow.puml', dark: true, status: 'Rendered locally', selectionRecordIds: ['record-7'] });
  assert.deepEqual(message, {
    type: 'detached-preview-state', svg: '<svg></svg>', filename: 'flow.puml', theme: 'dark', status: 'Rendered locally', selectionRecordIds: ['record-7']
  });
